...
int main(int argc, char **argv){
	...
	#if ( (2 < __GLIBC__) || ((2 == __GLIBC__) && (12 <= __GLIBC_MINOR__)) )
		int n1 = 0, n2 = 0;
		//qb->nDataLen = __libc_res_nsearch(resdup, querystr, C_IN, type, qb->ptDnsData->pBuffer, MAX_PACKET_SIZE - 1, NULL, NULL, &n1, &n2);
		int nDataLen =__libc_res_nsearch(res, psDnsQuery, C_IN, ns_t_ptr, pBuffer, MAX_PACKET_SIZE - 1, NULL, NULL, &n1, &n2);
	#else
		//qb->nDataLen = __libc_res_nsearch(resdup, querystr, C_IN, type, qb->ptDnsData->pBuffer, MAX_PACKET_SIZE - 1, NULL);
